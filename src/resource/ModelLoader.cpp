...
Entity ModelLoader::load(Scene& scene, const std::string& path) {
    ...
    if (!aiScene || aiScene->mFlags & AI_SCENE_FLAGS_INCOMPLETE ||
        !aiScene->mRootNode) {
      Logger::get()->error("Failed to load model: {}",
...
void ModelLoader::processNode(const aiNode * node, const aiScene* aiScene, Scene& scene, Entity parent) {
...
Entity ModelLoader::processMesh(const aiMesh * mesh, const aiScene* aiScene, Scene& scene, Entity parent) {
