...
FrameBuffer::FrameBuffer(const int width, const int height)
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB,
               GL_UNSIGNED_BYTE, nullptr); // Allocate texture memory
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                  GL_LINEAR); // Linear filtering for smooth results
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glBindTexture(GL_TEXTURE_2D, 0); // Unbind texture
  glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                         mTextureID, 0); // Attach to FBO

  // Create depth/stencil attachment as a renderbuffer (we don't need to sample
  // from it)
  ...
  glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width,
                        height); // Allocate depth+stencil storage
  glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                            GL_RENDERBUFFER, mRboID); // Attach to FBO
  ...
  glBindFramebuffer(GL_FRAMEBUFFER,
                    0); // Unbind framebuffer (return to default)
}
