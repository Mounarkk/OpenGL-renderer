 ...
 * 
 * The VertexArray class encapsulates an OpenGL VAO, which stores the configuration
 * of vertex attributes for efficient rendering. It manages the binding of vertex
 * buffers and their associated layouts, allowing for clean separation between
 * vertex data and attribute configuration. This class follows RAII principles
 * with proper move semantics and automatic resource cleanup.
 * 
 * Key features:
...
public:
   ...
   * All subsequent draw calls will use this VAO's vertex attribute configuration.
   */
  void bind() const;
