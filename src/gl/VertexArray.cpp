...
void VertexArray::addBuffer(const VertexBuffer& vb, const VertexBufferLayout& layout) const {
  ...
  for (unsigned int i = 0; i < elements.size(); ++i) {
    ...
    glVertexAttribPointer(i,                                           // attribute index
                          static_cast<GLint>(count),                   // number of components (1-4)
                          type,                                        // data type (float, int, etc.)
                          normalized ? GL_TRUE : GL_FALSE,             // normalize integers to [0,1] or [-1,1]
                          static_cast<GLsizei>(layout.getStride()),    // bytes between vertices
                          reinterpret_cast<const void*>(offset));      // offset to this attribute in vertex
    
    offset += count * VertexBufferElement::getTypeSize(type);  // Move to next attribute
  }
}

void VertexArray::bind() const {
  glBindVertexArray(mRendererId);  // Make this VAO active for subsequent operations
}
