...
/**
 * OpenGL shader program wrapper that handles loading, compilation, and uniform
 * management.
 *
 * The Shader class encapsulates an OpenGL shader program consisting of vertex
 * and fragment shaders. It handles the complete shader lifecycle: loading
 * source code from files, compiling individual shaders, linking the program,
 * and providing a convenient interface for setting uniform variables. The class
 * also includes uniform location caching for performance optimization.
 *
 * Key features:
...
public:
   ...
   * Loads, compiles, and links the shaders. Throws an exception if compilation
   * fails.
...
private:
  ...
  std::unordered_map<std::string, GLint>
      mUniformLocationCache; // Cache for uniform locations
