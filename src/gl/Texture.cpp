...
Texture::Texture(const std::string &path, const bool sRGB) {
  ...
  stbi_set_flip_vertically_on_load(true);  // OpenGL expects (0,0) at bottom-left, images have (0,0) at top-left
  unsigned char *data =
      stbi_load(path.c_str(), &mWidth, &mHeight, &mChannels, 0);  // Load with original channel count
  if (!data) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, mWidth, mHeight, 0, format, // Upload image data to GPU
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,                       // Use trilinear filtering when minifying
...
void Texture::bind(const GLuint slot) const {
  glActiveTexture(GL_TEXTURE0 + slot);  // Activate the specified texture unit (0-31)
