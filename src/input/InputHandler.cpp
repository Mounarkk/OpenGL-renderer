...
void InputHandler::handleMouseMovement(double xPos, double yPos, Camera& camera) {
    if (m_FirstMouse) {
    ...
    const float yOffset = m_LastY - static_cast<float>(yPos); // Reversed since y-coordinates go from bottom to top
...
void InputHandler::handleMouseScroll(double xOffset, double yOffset, Camera& camera) {
